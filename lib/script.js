...
let content = [
    ...
    {
        word:"erudite",
        def:"having or showing great knowledge"
    },
    {
        word:"ostentation",
...
function match(){
    ...
    if (showingCards[0].className==showingCards[1].className){
        ...
        showingCards=[]
        cards.forEach(card=>{
            if (card.children[0].classList.contains("pair")){
                pairedSum+=1} 
        })
        console.log(end.style.display)
        if (pairedSum==((0.25*cards.length**2)+(0.5*cards.length))){
...
function border(){
    cardFillers.forEach(cardFiller => {
        if (cardFiller.classList.contains("hide")==false){
            cardFiller.parentElement.style.boxShadow = "inset 0 0 0 6px #108f76"
        }
    })
}
